...
CREATE TRIGGER match_goals_set_updated_at BEFORE UPDATE ON public.match_goals FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

CREATE OR REPLACE FUNCTION public.validate_match_teams(p_round_id UUID, p_draw_id UUID, p_team_a_id UUID, p_team_b_id UUID)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE draw_round_id UUID; team_a_draw_id UUID; team_b_draw_id UUID;
BEGIN
...
CREATE OR REPLACE FUNCTION public.create_match(p_round_id UUID, p_draw_id UUID, p_team_a_id UUID, p_team_b_id UUID, p_scheduled_at TIMESTAMPTZ DEFAULT NULL, p_notes TEXT DEFAULT NULL)
...
BEGIN
  ...
  INSERT INTO public.matches (round_id, draw_id, team_a_id, team_b_id, scheduled_at, notes) VALUES (p_round_id, p_draw_id, p_team_a_id, p_team_b_id, p_scheduled_at, NULLIF(trim(p_notes), '')) RETURNING id INTO new_match_id;
  RETURN new_match_id;
END; $$;

CREATE OR REPLACE FUNCTION public.start_match(p_match_id UUID) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN RAISE EXCEPTION 'Only admins can start matches'; END IF;
  UPDATE public.matches SET status = 'in_progress', started_at = COALESCE(started_at, now()) WHERE id = p_match_id AND status = 'scheduled';
  IF NOT FOUND THEN RAISE EXCEPTION 'Match cannot be started'; END IF;
END; $$;

CREATE OR REPLACE FUNCTION public.set_match_score(p_match_id UUID, p_score_a INTEGER, p_score_b INTEGER) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
BEGIN
  ...
  SELECT count(*) FILTER (WHERE team_id = m.team_a_id), count(*) FILTER (WHERE team_id = m.team_b_id) INTO goal_a, goal_b FROM public.matches m LEFT JOIN public.match_goals g ON g.match_id = m.id WHERE m.id = p_match_id GROUP BY m.team_a_id, m.team_b_id;
...
CREATE OR REPLACE FUNCTION public.register_match_goal(p_match_id UUID, p_player_id UUID, p_team_id UUID, p_minute INTEGER DEFAULT NULL) RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE match_row public.matches%ROWTYPE; goal_id UUID;
BEGIN
...
CREATE OR REPLACE FUNCTION public.update_match_goal(p_goal_id UUID, p_player_id UUID, p_team_id UUID, p_minute INTEGER DEFAULT NULL) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
CREATE OR REPLACE FUNCTION public.delete_match_goal(p_goal_id UUID) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE goal_row public.match_goals%ROWTYPE; match_row public.matches%ROWTYPE;
BEGIN
...
CREATE OR REPLACE FUNCTION public.finish_match(p_match_id UUID) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE match_row public.matches%ROWTYPE; goal_a INTEGER; goal_b INTEGER;
BEGIN
...
CREATE OR REPLACE FUNCTION public.cancel_match(p_match_id UUID) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
