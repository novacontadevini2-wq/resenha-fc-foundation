...
import { ArrowLeft, CirclePlay, Flag, Goal, Handshake, XCircle } from "lucide-react";
...
function MatchDetailsPage() {
  ...
  async function callAction(action: "start_match" | "finish_match" | "cancel_match" | "delete_match") {
    if (!match) return;
    if (action === "cancel_match" && !window.confirm("Tem certeza que deseja cancelar esta partida?")) return;
    if (action === "delete_match" && !window.confirm("Tem certeza que deseja excluir esta partida? Todos os gols e assistências serão removidos.")) return;
    setWorking(true);
  ...
  return <AppLayout title="Detalhe da partida" subtitle={round ? `Rodada de ${new Date(`${round.scheduled_date}T12:00:00`).toLocaleDateString("pt-BR")}` : "Resenha FC"}>
    ...
    <section className="card-surface mb-5 p-5 text-center"><div className="flex items-center justify-between gap-3 text-left"><div><p className="text-meta">{match.scheduled_at ? new Date(match.scheduled_at).toLocaleString("pt-BR") : round?.start_time?.slice(0, 5) ?? "Horário não informado"}</p><p className="text-meta">{round?.location_name ?? "Rodada"}</p></div><div className="flex flex-col items-end gap-2"><MatchStatusBadge status={match.status} /><MatchTimer status={match.status} startedAt={match.started_at} finishedAt={match.finished_at} /></div></div><div className="mt-8 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><div className="font-display text-2xl font-bold text-navy">{teamName(match.team_a_id)}</div><div className="font-display text-5xl font-bold text-navy"><span>{match.status === "scheduled" || match.status === "cancelled" ? "-" : match.score_a}</span><span className="mx-2 text-2xl text-orange">x</span><span>{match.status === "scheduled" || match.status === "cancelled" ? "-" : match.score_b}</span></div><div className="font-display text-2xl font-bold text-navy">{teamName(match.team_b_id)}</div></div>{match.notes ? <p className="text-meta mt-5">{match.notes}</p> : null}<div className="mt-6 flex flex-wrap justify-center gap-2">{isAdmin && match.status === "scheduled" ? <Button onClick={() => void callAction("start_match")} disabled={working}><CirclePlay /> Iniciar partida</Button> : null}{isAdmin && match.status === "in_progress" ? <Button variant="outline" onClick={() => void callAction("finish_match")} disabled={working}><Flag /> Finalizar partida</Button> : null}{isAdmin && ["scheduled", "in_progress"].includes(match.status) ? <Button variant="ghost" onClick={() => void callAction("cancel_match")} disabled={working}><XCircle /> Cancelar partida</Button> : null}</div></section>
    <SectionCard title="Assistências" icon={Handshake} className="mb-5"><div className="grid gap-2">{assists.length ? assists.map((assist) => <div key={assist.id} className="flex items-center justify-between gap-2 border-b border-border pb-2 text-sm"><span>{playerName(assist.player_id)}</span><span className="text-meta">assistiu {playerName(goals.find((goal) => goal.id === assist.goal_id)?.player_id ?? "")}</span></div>) : <p className="text-meta">Nenhuma assistência registrada.</p>}</div></SectionCard>
    ...
    {canEditGoals ? <div className="mt-5 grid gap-5"><SectionCard title={editingGoal ? "Corrigir gol" : "Registrar gol"} icon={Goal}><form onSubmit={saveGoal} className="grid gap-3 sm:max-w-md"><label className="grid gap-1 text-sm font-medium text-navy">Equipe<Select value={selectedTeam} onValueChange={(value) => { setSelectedTeam(value); setSelectedPlayer(""); setSelectedAssist(""); }}><SelectTrigger><SelectValue placeholder="Selecione a equipe" /></SelectTrigger><SelectContent><SelectItem value={match.team_a_id}>{teamName(match.team_a_id)}</SelectItem><SelectItem value={match.team_b_id}>{teamName(match.team_b_id)}</SelectItem></SelectContent></Select></label><label className="grid gap-1 text-sm font-medium text-navy">Jogador<Select value={selectedPlayer} onValueChange={setSelectedPlayer} disabled={!selectedTeam}><SelectTrigger><SelectValue placeholder="Selecione o jogador" /></SelectTrigger><SelectContent>{availablePlayers.map((player) => <SelectItem key={player.player_id} value={player.player_id}>{player.player_name_snapshot}</SelectItem>)}</SelectContent></Select></label><label className="grid gap-1 text-sm font-medium text-navy">Assistência (opcional)<Select value={selectedAssist} onValueChange={setSelectedAssist} disabled={!selectedPlayer}><SelectTrigger><SelectValue placeholder="Sem assistência" /></SelectTrigger><SelectContent><SelectItem value="none">Sem assistência</SelectItem>{availablePlayers.filter((player) => player.player_id !== selectedPlayer).map((player) => <SelectItem key={player.player_id} value={player.player_id}>{player.player_name_snapshot}</SelectItem>)}</SelectContent></Select></label><label className="grid gap-1 text-sm font-medium text-navy">Minuto (opcional)<Input type="number" min="0" step="1" value={minute} onChange={(event) => setMinute(event.target.value)} /></label><p className="text-meta">O placar é atualizado automaticamente ao registrar ou remover gols.</p><div className="flex gap-2"><Button disabled={working}>{editingGoal ? "Salvar correção" : "Registrar gol"}</Button>{editingGoal ? <Button type="button" variant="ghost" onClick={resetGoalForm}>Cancelar</Button> : null}</div></form></SectionCard></div> : null}
    ...
    {isAdmin ? <Button variant="destructive" className="mt-5" onClick={() => void callAction("delete_match")} disabled={working}>Excluir partida</Button> : null}
  </AppLayout>;
}
