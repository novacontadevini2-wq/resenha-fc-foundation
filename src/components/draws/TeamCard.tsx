    ...
    <article className="card-surface min-w-0 p-4">
      <div className="mb-3 flex min-w-0 items-center justify-between gap-2">
        ...
        <span className="shrink-0 text-sm font-semibold text-orange">
          Força: {totalRating.toFixed(1)}
        </span>
      </div>
      <div className="team-card-grid grid grid-cols-2 gap-3">
        {players.map((snapshot) => (
          <div key={snapshot.player_id} className="min-w-0">
            <PlayerCard
              player={
                {
                  id: snapshot.player_id,
                  user_id: null,
                  name: snapshot.player_name_snapshot,
                  nickname: null,
                  photo_url: snapshot.photo_url_snapshot,
                  shirt_number: null,
                  status: "active",
                  overall_rating: snapshot.rating_snapshot,
                  created_at: "",
                  updated_at: "",
                } as Player
              }
              positions={snapshot.position_code_snapshot ? [snapshot.position_code_snapshot] : []}
              interactive={false}
            />
          </div>
        ))}
